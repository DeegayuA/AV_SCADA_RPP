...
export const plantConfigSchema = z.object({
  ...
  // Snap7 configuration fields
  snap7PlcIP: z.string().ip({ version: "v4", message: "Invalid IPv4 address" }).optional().or(z.literal('')),
  snap7PlcRack: z.coerce.number().min(0).max(7).default(0),
  snap7PlcSlot: z.coerce.number().min(0).max(31).default(2),
  snap7RemoteIP: z.string().ip({ version: "v4", message: "Invalid IPv4 address" }).optional().or(z.literal('')),
  snap7RemotePort: z.coerce.number().min(1024).max(65535).optional(),
  selectedProtocol: z.enum(['opcua', 'snap7']).default('opcua'),
