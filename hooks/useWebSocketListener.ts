    ...
    const connect = useCallback(() => {
        ...
        const handleMessage = (jsonString: string) => {
             try {
                ...
                if (message.requestId && connectionManager.pendingRequests.has(message.requestId)) {
...
function requestWithResponse<T>(message: WebSocketMessageToServer, timeout = 10000): Promise<T> {
    ...
    useEffect(() => {
        ...
        if (!state.sendJsonMessage) {
            state.setSendJsonMessage(sendJsonMessage);
        }
        if (!state.requestWithResponse) {
            state.setRequestWithResponse(requestWithResponse);
        }
