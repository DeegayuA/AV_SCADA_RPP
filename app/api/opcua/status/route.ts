...
let isSingleEndpointTestInProgress = false; // prevent overlapping test connections
...
export async function GET(request: NextRequest) {
    ...
    if (testedEndpointUrl) {
        ...
        // If a test is already running, avoid spawning multiple sessions
        if (isSingleEndpointTestInProgress) {
            return NextResponse.json({
                connectionStatus: 'disconnected',
                message: 'A connection test is already in progress. Please wait and retry.',
                testedEndpoint: testedEndpointUrl,
            }, { status: 429 });
        }

        // If the main client is already connected to this endpoint and session is valid, reuse its status
        try {
            if (
                mainClient &&
                mainSession &&
                !mainSession.sessionId.isEmpty() &&
                (mainClient as any).endpointUrl === testedEndpointUrl &&
                mainCurrentConnectionStatus !== 'disconnected'
            ) {
                return NextResponse.json({
                    connectionStatus: mainCurrentConnectionStatus,
                    message: `Using existing main client session for ${testedEndpointUrl}. Status: ${mainCurrentConnectionStatus}`,
                    testedEndpoint: testedEndpointUrl,
                });
            }
        } catch (_) { /* fall through to fresh test */ }

        let tempTestClient: OPCUAClient | null = null;
        let tempTestSession: ClientSession | null = null;
        isSingleEndpointTestInProgress = true;
        ...
        try {
            ...
            // Always cleanup the temporary test client/session to avoid server-side stale sessions
            try { if (tempTestSession) await tempTestSession.close(); } catch (e: any) { console.warn('Error closing temp test session (finally):', e.message); }
            try { if (tempTestClient) await tempTestClient.disconnect(); } catch (e: any) { console.warn('Error disconnecting temp test client (finally):', e.message); }
            isSingleEndpointTestInProgress = false;
        }
