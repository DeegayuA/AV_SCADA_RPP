...
export async function GET() {
  try {
    ...
    const logsDir = path.join(process.cwd(), "logs", "notes");
    ...
    for (const file of files) {
      ...
      for (const line of lines) {
        try {
          ...
          allLogs.push({ ...obj, uploadType: "note", date: obj.timestamp });
        } catch (e) {
    ...
    console.error("Error reading note logs:", error);
    return NextResponse.json(
      { message: "Error reading note logs" },
      { status: 500 }
    );
  }
}
