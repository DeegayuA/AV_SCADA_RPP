...
const TransformerNode: React.FC<NodeProps<TransformerNodeData>> = (props) => {
  ...
  const statusUiStyles = useMemo(() => {
    ...
    if (processedStatus === 'fault' || processedStatus === 'alarm') {
      baseBorderClass = 'border-red-500/80 dark:border-red-500/70'; iconColorClass = 'text-red-500 dark:text-red-400';
      textColorClass = 'text-red-600 dark:text-red-400 font-semibold'; additionalInfoColorClass = iconColorClass;
      glowRgb = '239, 68, 68';
    } else if (processedStatus === 'warning') {
      baseBorderClass = 'border-amber-500/80 dark:border-amber-400/70'; iconColorClass = 'text-amber-500 dark:text-amber-400';
      textColorClass = 'text-amber-600 dark:text-amber-400 font-medium'; additionalInfoColorClass = iconColorClass;
      glowRgb = '245, 158, 11';
    } else if (isTransformerEnergized) {
      baseBorderClass = 'border-teal-500/80 dark:border-teal-400/70'; iconColorClass = isDarkMode ? 'text-teal-300' : 'text-teal-500';
      textColorClass = isDarkMode ? 'text-teal-200' : 'text-teal-600'; additionalInfoColorClass = iconColorClass;
      glowRgb = isDarkMode ? '45, 212, 191' : '20, 184, 166';
    }
    
    return { baseBorderClass, iconColorClass, textColorClass, baseBgStartColor, baseBgEndColor, additionalInfoColorClass, glowColor: `rgba(${glowRgb}, 0.45)`, activeGlowColor: `rgba(${glowRgb}, ${isTransformerEnergized && !isCriticalStatus ? 0.35 : 0})`, };
  ...
  const nodeMainStyle = useMemo((): MotionStyle => {
    ...
    return {
      ...dynamicBgStyle,
      borderColor: derivedNodeStyles.borderColor || statusUiStyles.baseBorderClass.split(' ').pop() || 'transparent',
  ...
  }, [statusUiStyles, derivedNodeStyles, selected, isRecentStatusChange, nodeWidthFromData, nodeHeightFromData, electricCyan]);
  ...
  const TransformerSymbolSVG = ({ className, isEnergized }: {className?: string, isEnergized?: boolean}) => {
    // --- FIX: Explicitly type variants and transition ---
    const coilTransition: Transition = { duration: 1.8, repeat: Infinity, ease: "easeInOut" };
    const coilVariants: Variants = {
      energized: { opacity: [0.6, 1, 0.6], transition: coilTransition },
      offline: { opacity: 1 },
    };
    const strokeColor = derivedNodeStyles.color || statusUiStyles.iconColorClass;
    ...
    return (
      <motion.svg viewBox="0 0 24 24" width="30" height="30" className={className} initial={false} style={{ color: strokeColor}}>
        ...
        <motion.circle 
          ...
          variants={coilVariants} animate={isEnergized ? "energized" : "offline"}
          transition={{ ...coilTransition, delay: 0.4 }} 
        />
  ...
  const mainDivTransition: Transition = (isTransformerEnergized && !isCriticalStatus && !selected && !isRecentStatusChange)
  ...
  const statusTextTransition: Transition = { duration: 0.18, ease: "easeInOut" };
  const criticalIconTransition: Transition = { duration: 1.5, repeat: Infinity, ease: "easeInOut" };
  
  return (
    <motion.div
      className={` transformer-node group sld-node relative flex flex-col items-center justify-between rounded-lg border backdrop-blur-sm transition-colors duration-300 ease-in-out ${statusUiStyles.baseBorderClass} ${isNodeEditable ? 'cursor-grab' : 'cursor-pointer'} ${selected ? `ring-2 ring-offset-2 ring-[${electricCyan}] dark:ring-[${electricCyan}] ring-offset-black/10 dark:ring-offset-white/10 shadow-lg` : 'shadow-md'} overflow-hidden `}
      style={{ ...nodeMainStyle, background: `linear-gradient(to bottom, var(--bg-start-color), var(--bg-end-color))` }}
      initial={{ opacity: 0, scale: 0.88, y: 15 }}
      animate={{ opacity: 1, scale: 1, y: 0, boxShadow: (isTransformerEnergized && !isCriticalStatus && !selected && !isRecentStatusChange) ? [ `0 0 8px 1.5px ${statusUiStyles.activeGlowColor.replace('0.35', '0.3')}`, `0 0 12px 2.5px ${statusUiStyles.activeGlowColor.replace('0.35', '0.5')}`, `0 0 8px 1.5px ${statusUiStyles.activeGlowColor.replace('0.35', '0.3')}` ] : String(nodeMainStyle.boxShadow) }} 
      exit={{ opacity: 0, scale: 0.85, transition: { duration: 0.15, ease: "easeOut"} }}
      transition={mainDivTransition}
      whileHover={{ scale: isNodeEditable ? 1.035 : 1.02, boxShadow: selected ? `0 0 18px 3.5px ${electricCyan}, 0 0 6px 1.5px ${electricCyan} inset` : `0 0 15px 3.5px ${statusUiStyles.glowColor.replace('0.45', '0.6')}` }}
      onClick={(e) => { if (!isNodeEditable && !isEditMode) { e.stopPropagation(); setSelectedElementForDetails(fullNodeObjectForDetails); }}}
    >
      <Handle type="target" position={Position.Top} id="primary_in" isConnectable={isConnectable} className="!w-3 !h-3 !bg-slate-500/80 dark:!bg-slate-600/80 !border-slate-600 dark:!border-slate-500 shadow !rounded-full sld-handle-style hover:!scale-150 hover:!opacity-100 transition-all duration-150 z-10" title="Primary Input"/>
      <Handle type="source" position={Position.Bottom} id="secondary_out" isConnectable={isConnectable} className="!w-3 !h-3 !bg-slate-500/80 dark:!bg-slate-600/80 !border-slate-600 dark:!border-slate-500 shadow !rounded-full sld-handle-style hover:!scale-150 hover:!opacity-100 transition-all duration-150 z-10" title="Secondary Output"/>

      {!isEditMode && (<Button variant="ghost" size="icon" title="View Details" className="absolute top-1 right-1 h-6 w-6 rounded-full z-20 group/infobtn bg-slate-300/20 dark:bg-slate-700/20 hover:bg-slate-300/40 dark:hover:bg-slate-700/40 p-0 backdrop-blur-sm transition-all duration-200" onClick={(e) => { e.stopPropagation(); setSelectedElementForDetails(fullNodeObjectForDetails); }}><InfoIcon className={`h-3.5 w-3.5 text-slate-500 dark:text-slate-400 group-hover/infobtn:text-[${electricCyan}] dark:group-hover/infobtn:text-[${electricCyan}] transition-colors duration-150`} /> </Button>)}
      ...
      <div className="flex flex-col items-center justify-between w-full h-full px-1.5 py-1 pointer-events-none space-y-0.5">
        <motion.p
          ...
          title={data.label}
          style={{ color: derivedNodeStyles.color } as MotionStyle}
        >
          {data.label}
        </motion.p>
        ...
        <div className="h-3.5 overflow-hidden">
            <AnimatePresence mode="wait">
                <motion.p
                    ...
                    exit={{ opacity: 0, y: -4 }} transition={statusTextTransition}
                >
                    {displayStatusText}
                </motion.p>
            </AnimatePresence>
        ...
        <div className="flex-shrink-0 my-0.5">
            <motion.div animate={isCriticalStatus && DisplayIconComponent === AlertTriangleIcon ? { scale: [1, 1.06, 1], transition: criticalIconTransition } : {}}>
        ...
        <p className={`text-[9px] font-medium text-center truncate w-full leading-tight ${statusUiStyles.additionalInfoColorClass} transition-colors duration-200`} title={additionalInfo} style={{ color: derivedNodeStyles.color } as React.CSSProperties}>
          {additionalInfo}
        </p>
      </div>
    </motion.div>
