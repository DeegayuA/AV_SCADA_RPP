...
function getApplicableTargetProperties(element: CustomNodeType | CustomFlowEdge | null): TargetPropertyDefinition[] {
    ...
    if (isNode(element) && element.data) {
        ...
        // If the element is a Gauge, remove 'gaugeValue' from the applicableProps
...
const SLDInspectorDialog: React.FC<SLDInspectorDialogProps> = ({
    ...
    const handleGaugeValueDataPointFormatChange = useCallback((field: keyof NonNullable<DataPointLink['format']>, value: any) => {
        setFormData(prev => {
            ...
            if (newFormData.config && (newFormData.config as GaugeNodeData['config'])?.valueDataPointLink) {
                let currentFormat = (newFormData.config as GaugeNodeData['config'])!.valueDataPointLink!.format || { type: 'number' }; // Default to number for gauge

                currentFormat = { ...currentFormat, [field]: value };

                if (field === 'type') {
                    const dpId = (newFormData.config as GaugeNodeData['config'])!.valueDataPointLink!.dataPointId;
                    const selectedDp = dpId ? dataPoints[dpId] : null;

                    if (value !== 'number') {
                        delete currentFormat.precision;
                        // Only delete suffix if it was likely auto-set from DP unit and now type is not string/number
                        if (value !== 'string' && selectedDp?.unit && currentFormat.suffix === selectedDp.unit) {
                             delete currentFormat.suffix;
                        }
                    } else { // type is 'number'
                        if (currentFormat.precision === undefined) currentFormat.precision = 2; // Default precision
                        if (selectedDp?.unit && !currentFormat.suffix) currentFormat.suffix = selectedDp.unit; // Re-apply unit if not set
                    }
                    if (value !== 'boolean') {
                        delete currentFormat.trueLabel;
                        delete currentFormat.falseLabel;
                    }
                    if (value !== 'dateTime') {
                        delete currentFormat.dateTimeFormat;
                    }
                }
                (newFormData.config as GaugeNodeData['config'])!.valueDataPointLink!.format = currentFormat;
            }
            return newFormData;
        });
    }, [dataPoints]);
    ...
    useEffect(() => {
    if (isOpen && selectedElement && dataPoints['ns=4;i=224']) { // Use the actual ID string
        console.log("Inspecting DP ns=4;i=224 from store:", dataPoints['ns=4;i=224']);
        // Specifically log the isWritable flag
        console.log("Is ns=4;i=224 .isWritable?", dataPoints['ns=4;i=224']?.isWritable);
    }
}, [isOpen, selectedElement, dataPoints]); // Added dataPoints for logging context
                        ...
                        <Tabs defaultValue={activeTab} onValueChange={setActiveTab} className="w-full">
                            ...
                            <div className="p-4 md:p-5 lg:p-6">
                                <TabsContent value="properties" className="mt-0 space-y-5 outline-none">
                                    ...
                                    {isNode(selectedElement) && currentElementNodeSLDType === SLDElementType.Panel && (<Card className='shadow-md border-border/60'><CardHeader className='p-3.5'><CardTitle className='text-base font-semibold'>PV Panel Configuration</CardTitle></CardHeader><CardContent className='p-3.5 pt-0 space-y-3'>
                                        <div className="space-y-1"> <Label htmlFor="config.technology" className="text-xs">Technology</Label> <Select value={(formData.config as PanelNodeData['config'])?.technology || ''} onValueChange={(val) => handleSelectChange("config.technology", val)}> <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Select technology..." /></SelectTrigger> <SelectContent><SelectItem value="Mono-Si" className="text-xs">Mono-Si</SelectItem><SelectItem value="Poly-Si" className="text-xs">Poly-Si</SelectItem><SelectItem value="Thin-film" className="text-xs">Thin-film</SelectItem></SelectContent> </Select> </div>
                                        <FieldInput type="number" id="config.powerRatingWp" name="config.powerRatingWp" label="Power Rating (Wp)" value={(formData.config as PanelNodeData['config'])?.powerRatingWp ?? ''} onChange={handleInputChange} placeholder="e.g., 300" min="0" info="Peak power rating in Watt-peak." />
                                        <FieldInput type="number" id="config.numberOfPanels" name="config.numberOfPanels" label="Number of Panels in Array" value={(formData.config as PanelNodeData['config'])?.numberOfPanels ?? ''} onChange={handleInputChange} placeholder="e.g., 10" min="1" info="Specify the total number of panels this array represents." />
                                        </CardContent></Card>)}
    ...
    <Card className='shadow-md border-border/60'>
        <CardHeader className='p-3.5'>
            <CardTitle className='text-base font-semibold'>Battery System Configuration</CardTitle>
        </CardHeader>
        <CardContent className='p-3.5 pt-0 space-y-3'>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
                <div className="space-y-1"> 
                    <Label htmlFor="config.technology" className="text-xs">Technology</Label> 
                    <Select 
                        value={(formData.config as BatteryNodeData['config'])?.technology || ''} 
                        onValueChange={(val) => handleSelectChange("config.technology", val)}
                    > 
                        <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Select technology..." /></SelectTrigger> 
                        <SelectContent>
                            <SelectItem value="Li-ion" className="text-xs">Li-ion (Lithium-ion)</SelectItem>
                            <SelectItem value="LFP" className="text-xs">LFP (LiFePO4)</SelectItem>
                            <SelectItem value="Lead-Acid" className="text-xs">Lead-Acid</SelectItem>
                            <SelectItem value="Flow" className="text-xs">Flow Battery</SelectItem>
                            <SelectItem value="NiCd" className="text-xs">NiCd (Nickel-Cadmium)</SelectItem>
                ...
                <FieldInput 
                    type="number" 
                    id="config.capacityAh" 
                    name="config.capacityAh" 
                    label="Capacity (Ah)" 
                    value={(formData.config as BatteryNodeData['config'])?.capacityAh ?? ''} 
                    onChange={handleInputChange} 
                    placeholder="e.g., 1000" 
                    min="0"
                    info="Nominal capacity in Ampere-hours."
                />
                <FieldInput 
                    type="number" 
                    id="config.voltageNominalV" 
                    name="config.voltageNominalV" 
                    label="Nominal Voltage (V)" 
                    value={(formData.config as BatteryNodeData['config'])?.voltageNominalV ?? ''} 
                    onChange={handleInputChange} 
                    placeholder="e.g., 48" 
                    min="0"
                    info="Nominal DC voltage of the battery system."
                />
                <FieldInput 
                    type="number" 
                    id="config.dodPercentage" 
                    name="config.dodPercentage" 
                    label="Max Depth of Discharge (%)" 
                    value={(formData.config as BatteryNodeData['config'])?.dodPercentage ?? ''} 
                    onChange={handleInputChange} 
                    placeholder="e.g., 80" 
                    min="0" max="100"
                    info="Maximum allowed depth of discharge."
                />
                <FieldInput 
                    type="number" 
                    id="config.numModules" 
                    name="config.numModules" 
                    label="Number of Modules (Visual)" 
                    value={(formData.config as BatteryNodeData['config'])?.numModules ?? ''} 
                    onChange={handleInputChange} 
                    placeholder="e.g., 3" 
                    min="1" 
                    info="Number of visual modules in the battery bank display (e.g., 1-5)."
                />
                <FieldInput 
                    type="number" 
                    id="config.soc" 
                    name="config.soc" 
                    label="Manual SOC (%)" 
                    value={(formData.config as BatteryNodeData['config'])?.soc ?? ''} 
                    onChange={handleInputChange} 
                    placeholder="e.g., 75" 
                    min="0" max="100"
                    info="Manually set State of Charge (0-100%). A linked data point for 'Battery SOC (%)' will override this value if present."
                />
            </div>
        </CardContent>
    </Card>
                                    ...
                                    {isNode(selectedElement) && currentElementNodeSLDType === SLDElementType.Gauge && (<> <Card className='shadow-md border-border/60'><CardHeader className='p-3.5'><CardTitle className='text-base font-semibold'>Gauge Display Configuration</CardTitle></CardHeader><CardContent className='p-3.5 pt-0 space-y-3'><FieldInput type="number" id="config.minVal" name="config.minVal" label="Minimum Value" value={(formData.config as GaugeNodeData['config'])?.minVal ?? ''} onChange={handleInputChange} placeholder="e.g., 0" info="The lowest value the gauge can display."/><FieldInput type="number" id="config.maxVal" name="config.maxVal" label="Maximum Value" value={(formData.config as GaugeNodeData['config'])?.maxVal ?? ''} onChange={handleInputChange} placeholder="e.g., 100" info="The highest value the gauge can display."/><FieldInput id="config.unit" name="config.unit" label="Unit" value={(formData.config as GaugeNodeData['config'])?.unit ?? ''} onChange={handleInputChange} placeholder="e.g., %, kW, °C" info="The unit of measurement to display with the value."/></CardContent></Card> <Card className='shadow-md border-border/60'><CardHeader className='p-3.5'><CardTitle className='text-base font-semibold'>Gauge Value Data Point</CardTitle></CardHeader><CardContent className='p-3.5 pt-0 space-y-3'>
                                        ...
                                        <FieldInput
                                            ...
                                            onChange={(value: string | null) => {
                                                ...
                                                handleSelectChange("config.valueDataPointLink", value ? {
                                                    dataPointId: value,
                                                    targetProperty: 'value', // Default target for gauge main value
                                                    format: {
                                                        type: inferredType,
                                            ...
                                            info="Select the data point that will drive the gauge's value."
                                        />
                                        ...
                                        {(formData.config as GaugeNodeData['config'])?.valueDataPointLink?.dataPointId && (
                                            <div className="mt-2.5 pt-2.5 border-t border-border/40 space-y-2.5">
                                                <Label className="text-xs font-medium flex items-center text-muted-foreground">Selected Value Formatting
                                                    <Tooltip><TooltipTrigger type="button" className="ml-1.5 focus:outline-none"><InfoIconLucide className="w-3 h-3 text-muted-foreground/60 cursor-help"/></TooltipTrigger><TooltipContent side="top" className="max-w-xs"><p>Define how the selected data point's value should be formatted before being displayed on the gauge or used in calculations if applicable.</p></TooltipContent></Tooltip>
                                                </Label>
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 items-start bg-muted/20 dark:bg-gray-800/20 p-2.5 rounded-md">
                                                    <div className="space-y-1">
                                                        <Label htmlFor="gaugeValueFormat.type" className="text-[10px] font-medium">Interpret As</Label>
                                                        <Select
                                                            value={(formData.config as GaugeNodeData['config'])?.valueDataPointLink?.format?.type || 'number'}
                                                            onValueChange={(val) => handleGaugeValueDataPointFormatChange('type', val as NonNullable<DataPointLink['format']>['type'])}
                                                        >
                                                            <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                                                            <SelectContent>
                                                                <SelectItem value="number" className="text-xs">Number</SelectItem>
                                                                <SelectItem value="string" className="text-xs">Text (String)</SelectItem>
                                                                <SelectItem value="boolean" className="text-xs">True/False (Boolean)</SelectItem>
                                                            </SelectContent>
                                                        </Select>
                                                    </div>
                                                    <div> {/* Spacer */} </div>
                                                    {(formData.config as GaugeNodeData['config'])?.valueDataPointLink?.format?.type === 'number' && (
                                                        <FieldInput
                                                            type="number"
                                                            id="gaugeValueFormat.precision"
                                                            name="gaugeValueFormat.precision" // Not directly used by handler, but good for consistency
                                                            label="Decimal Places"
                                                            value={(formData.config as GaugeNodeData['config'])?.valueDataPointLink?.format?.precision ?? ''}
                                                            onChange={(e:React.ChangeEvent<HTMLInputElement>) => handleGaugeValueDataPointFormatChange('precision', e.target.value === '' ? undefined : parseInt(e.target.value) || 0)}
                                                            placeholder="e.g., 2"
                                                            min="0"
                                                            max="10"
                                                        />
                                                    )}
                                                     {/* Suffix is applicable to number and potentially string */}
                                                     {((formData.config as GaugeNodeData['config'])?.valueDataPointLink?.format?.type === 'number' || (formData.config as GaugeNodeData['config'])?.valueDataPointLink?.format?.type === 'string') && (
                                                        <FieldInput
                                                            id="gaugeValueFormat.suffix"
                                                            name="gaugeValueFormat.suffix" // Not directly used by handler
                                                            label="Unit/Suffix"
                                                            value={(formData.config as GaugeNodeData['config'])?.valueDataPointLink?.format?.suffix ?? ''}
                                                            onChange={(e:React.ChangeEvent<HTMLInputElement>) => handleGaugeValueDataPointFormatChange('suffix', e.target.value)}
                                                            placeholder="e.g., kW, °C"
                                                        />
                                                    )}
                                                    {(formData.config as GaugeNodeData['config'])?.valueDataPointLink?.format?.type === 'boolean' && (
                                                        <>
                                                            <FieldInput
                                                                id="gaugeValueFormat.trueLabel"
                                                                label="Show TRUE as"
                                                                value={(formData.config as GaugeNodeData['config'])?.valueDataPointLink?.format?.trueLabel || 'True'}
                                                                onChange={(e:React.ChangeEvent<HTMLInputElement>) => handleGaugeValueDataPointFormatChange('trueLabel', e.target.value)}
                                                                placeholder="e.g., ON, Active"
                                                            />
                                                            <FieldInput
                                                                id="gaugeValueFormat.falseLabel"
                                                                label="Show FALSE as"
                                                                value={(formData.config as GaugeNodeData['config'])?.valueDataPointLink?.format?.falseLabel || 'False'}
                                                                onChange={(e:React.ChangeEvent<HTMLInputElement>) => handleGaugeValueDataPointFormatChange('falseLabel', e.target.value)}
