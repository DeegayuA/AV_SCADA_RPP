...
import { Settings, PlusCircle, Clock, Trash2, RotateCcw, Power, AlertTriangle, Check, ShieldAlert, InfoIcon as InfoIconLucide, Loader2, Maximize2, X, Pencil } from 'lucide-react'; // Added Maximize2, X
...
// DashboardHeaderControl and HeaderConnectivityComponent remain the same
...
const DashboardHeaderControl: React.FC<DashboardHeaderControlProps> = React.memo(
  ({
    ...
    // Handler for navigating to reset page
      ...
      <>
        ...
        >
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap justify-center">
            ...
            {/* Dashboard Layout Editing Tools (only in Edit Mode for Admin) */}
            {isAdmin && isEditMode && (
              <>
...
<span className="sm:hidden">Edit</span>
            ...
            {/* Admin Specific Global Controls */}
            {isAdmin && (
              <>
                ...
                {/* Navigate to Full Application Reset Page */}
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <motion.div variants={itemVariants}>
                      <TooltipProvider delayDuration={100}>
        ...
        {/* Current Time and Connection Lag */}
        <motion.div 
          ...
          <div className="flex items-center gap-2">
            ...
            {isConnected ? (
              <TooltipProvider delayDuration={100}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <span className={`font-mono cursor-default px-1.5 py-0.5 rounded text-xs ${
...
const UnifiedDashboardPage: React.FC = () => {
    ...
    const [isSldModalOpen, setIsSldModalOpen] = useState(false); // State for SLD Modal
    ...
    }, [playNotificationSound, maxReconnectAttempts, WS_URL, authCheckComplete]); // Added authCheckComplete to dependencies
    ...
    const sldHeaderButtonHeight = sldSpecificEditMode ? '2.75rem' : '0rem'; 
        ...
        <div className="bg-background text-foreground px-3 sm:px-4 md:px-6 lg:px-8 transition-colors duration-300 pb-8">
            ...
            <Dialog open={isSldModalOpen} onOpenChange={setIsSldModalOpen}>
                <DialogContent className="sm:max-w-[90vw] w-[95vw] h-[90vh] p-0 flex flex-col dark:bg-background bg-background border dark:border-slate-800">
                    ...
                    <div className="flex-grow p-2 sm:p-4 overflow-hidden"> {/* Container for SLDWidget */}
                        <SLDWidget
                            layoutId={sldLayoutId}
                            isEditMode={false} // Usually, popup view is not for editing node positions
                            // Assuming SLDWidget can adapt to its parent's size.
                            // If it needs explicit dimensions, they might need to be calculated
                            // or passed as props (e.g., based on the modal content area size).
                        />
                    </div>
                </DialogContent>
            </Dialog>
