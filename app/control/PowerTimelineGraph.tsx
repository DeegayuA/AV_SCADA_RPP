...
const getResolvedColor = (
    ...
    if (typeof window === 'undefined' || !document.documentElement.style.getPropertyValue('--primary')) {
        const ssrFallbacks: Record<string, string> = { 
            generation: "hsl(200,80%,50%)", usage: "hsl(55, 89.80%, 50.00%)", gridFeed: "hsl(280,70%,60%)", 
    ...
    if (mappedColorInfo?.cssVar) {
        ...
        if (!resolvedColorValue) resolvedColorValue = chartConfigEntry.directFallback; // Fallback to directFallback first
    }
    ...
    if (!resolvedColorValue) {
        ...
        if (mappedColor?.direct) {
            fallback = mappedColor.direct;
             ...
             const baseConfig = baseChartConfig[cName as keyof typeof baseChartConfig];
             if (baseConfig?.directFallback) {
                fallback = baseConfig.directFallback;
             }
        }
        resolvedColorValue = fallback || "hsl(0,0%,50%)"; // Final default grey
    }
...
const PowerTimelineGraph: React.FC<PowerTimelineGraphProps> = ({
    ...
    const formatXAxisTick = useCallback((ts: number) => {
        ...
        switch(timeScale) {
            ...
            case '7d': return date.toLocaleDateString([], { weekday: 'short', day: 'numeric' }); 
            case '1d': return date.toLocaleTimeString([],{hour12:false, hour:'numeric', minute:'2-digit'});
            case '12h': case '6h': case '1h':  case '30m':
    ...
    const disablePrevButton = useMemo(() => {
        ...
        const currentWindowEndTime = Date.now() - historicalTimeOffsetMs;
        const prospectiveNewWindowStartTime = (currentWindowEndTime - shiftAmount) - durationMs;
        // Check if the *start* of the proposed shifted window is before the oldest data point.
        return prospectiveNewWindowStartTime < oldestDataPointTimestamp;
          ...
          if (i > 0) { 
            ...
            if ((prevPoint.gridFeed >=0 && point.gridFeed < 0) || (prevPoint.gridFeed < 0 && point.gridFeed >=0)) { 
              ...
              if (t1 === t0 || v1 === v0) { 
                ...
                const factor = (t1 - t0 === 0) ? 0 : (tZero - t0) / (t1 - t0); // Avoid division by zero if t1 equals t0
    ...
    const legendPayloadForChartLegend = useMemo((): LegendPayload[] => {
        const items: ActualLineKey[] = [];
        if (visibleLines.generation) items.push('generation');
        if (visibleLines.usage) items.push('usage');
        if (visibleLines.gridFeed) items.push('gridFeed');

        return items.map((key) => {
            const config = baseChartConfig[key];
            return {
                dataKey: key, // Used by ChartLegendContent to find label in chartConfig
                value: config.label, // Text label for display (used if ChartLegendContent uses it)
                payload: { // Recharts internal payload structure often includes icon and label like this
                    label: config.label,
                    icon: config.icon, // Icon from our baseChartConfig
                },
                color: getResolvedColor(key, resolvedTheme), // Base color for the legend icon
                type: 'line', // Legend icon type (e.g., line, square, circle)
                inactive: !visibleLines[key], // Used for styling inactive items if legend supports it
            };
        });
                   ...
                   {isGraphReady && dpsAreConfigured && (chartData.length > 0 || !isCurrentlyDrawingLiveOrDemo) ? ( 
                    <ChartContainer config={baseChartConfig} className="w-full h-full">
                        <ComposedChart accessibilityLayer data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                                ...
                                content={
                                    <ChartTooltipContent
                                        ...
                                        labelFormatter={(value, payload) => { 
                                            ...
                                            switch(timeScale){
                                                case '1mo': return date.toLocaleString([], { year:'numeric', month:'short', day:'numeric', hour:'numeric', minute:'2-digit' });
                                                case '7d': return date.toLocaleString([], { month:'short', day:'numeric', weekday:'short', hour:'numeric', minute:'2-digit' });
                                                case '1d': case '12h': case '6h': return date.toLocaleString([], { day:'numeric', month:'short', hour:'numeric',minute:'2-digit', second:'2-digit' });
                                                case '1h':  case '30m': case '5m': return date.toLocaleTimeString([], {hour:'numeric',minute:'2-digit', second:'2-digit'});
                                                default: return date.toLocaleTimeString([], {hour:'numeric',minute:'2-digit',second:'2-digit', fractionalSecondDigits:1} as any);
                                            }
                                        }}
                                        formatter={(value, name, item, index, payloadProp) => {
                                            ...
                                            const result = ( <div className="flex items-center gap-2.5" key={`${name as string}-${index}`}> {Icon && <Icon className="h-4 w-4 shrink-0" style={{ color: itemColor }} />} <div className="flex flex-1 justify-between leading-none"> <span className="text-muted-foreground">{displayLabel}</span> <span className="font-bold" style={{ color: itemColor }}> {displayValue.toFixed(valuePrecision)} <span className="ml-1 font-normal text-muted-foreground">{displayUnitLabel}</span> </span> </div> </div> );
                                            
                                            // Add net power calculation as the last item
                                            if (Array.isArray(payloadProp) && index === payloadProp.length - 1 && payloadProp[0]?.payload) {
                                                const { generation, usage } = payloadProp[0].payload as ChartDataPoint;
                                                const netPower = Math.abs(generation) - Math.abs(usage);
                            ...
                            { chartData.length > 3 && 
                              <ChartLegend 
                                payload={legendPayloadForChartLegend} // Use custom payload
                            ...
                            {visibleLines.gridFeed && gridFeedSegments.map((segment, index) => (
                                <Line
                                    ...
                                    name={baseChartConfig.gridFeed.label} // Name is still useful for tooltip item identification
                                    ...
                                    isAnimationActive={isGraphReady && isCurrentlyDrawingLiveOrDemo} animationDuration={effectiveUseDemoData ? 200 : 120} connectNulls={false} 
                                    legendType="none" // Not strictly necessary if ChartLegend uses a custom payload, but good practice
                                />
                            ))}
                        </ComposedChart>
                    </ChartContainer>
