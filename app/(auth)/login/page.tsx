...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const checkOnboardingAndUser = async () => {
      ...
      // STRICT: Admin users ALWAYS go through onboarding regardless of completion status
      if (directStoreUser && directStoreUser.role === UserRole.ADMIN && directStoreUser.email !== 'guest@example.com') {
        if (!completed) {
          toast.info("Initial Setup Required", { description: "Please complete all onboarding steps to access the dashboard.", duration: 4000 });
        } else {
          toast.info("Onboarding Access", { description: "You can review your configuration or access the dashboard from the final step.", duration: 4000 });
        }
        router.replace('/onboarding');
        return;
      }

      // For non-admin users, show login form only if onboarding is complete
      if (!completed) {
