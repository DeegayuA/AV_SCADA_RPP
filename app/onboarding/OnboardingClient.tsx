            ...
            try {
                ...
                if (user.role === UserRole.ADMIN) {
                    if (wantsReset) {
                        ...
                        // STRICT: Always show onboarding until ALL steps are completed
                        // Users MUST go through every single step sequentially
                        if (idbOnboardingCompleted) {
                            toast.info("Configuration Review", { description: "You can review your setup or make changes before accessing the dashboard.", duration: 4000 });
                        } else {
                            toast.info("Setup Required", { description: "Please complete all onboarding steps to access the dashboard.", duration: 4000 });
                        }
                        setPageState('onboarding_active');
                    }
                } else { 
                    if (!idbOnboardingCompleted) {
                        ...
                        toast.warning("Admin Setup Required", { description: "Please contact an administrator to complete initial setup.", duration: 5000 });
                    } else {
                        // For non-admin users, still redirect when complete
